	...
	return (
		<div style={{ background: '#fcfcfc', minHeight: '100vh', padding: '2rem 1rem' }}>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            ...
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={load} style={{ background: RED, color: '#fff', border: 'none', padding: '10px 20px', borderRadius: 12, fontSize: 13, fontWeight: 700, cursor: 'pointer', boxShadow: `0 8px 20px rgba(196,30,58,0.2)` }}>
              Refresh Data
            </motion.button>
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={onLogout} style={{ background: 'none', color: '#111', border: '1.5px solid #eee', padding: '10px 20px', borderRadius: 12, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
              Logout
            </motion.button>
          </div>
				</header>
              ...
              >
                {products.map(p => (
                  <motion.div 
                    ...
                    whileHover={{ scale: 1.015, boxShadow: '0 10px 30px rgba(0,0,0,0.06)', borderColor: CYAN }}
                    key={p._id || p.id} 
                    style={{ background:'#fff', border:'1.5px solid #eee', borderRadius:20, padding:15, display:'flex', gap:15, alignItems:'center', transition:'border-color 0.2s ease' }}
                  >
                    ...
                    <div style={{ display:'flex', gap:8 }}>
                      <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={()=>setEditingProduct(p)} style={{ width:36, height:36, borderRadius:10, border:'1.5px solid #eee', background:'#fff', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>
                      ...
                      <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={()=>onDelete(p._id || p.id)} style={{ width:36, height:36, borderRadius:10, border:'none', background:'#fde8ec', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>
